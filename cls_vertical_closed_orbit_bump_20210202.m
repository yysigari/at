%% Code for simulating a vertical bump orbi at the XSR
%setorbitbumpgui source point
%-- 2021-02-02, 13:35 --%
...
%%
switch2sim % see below, can use setorbitbump with command not GUI
...
% get the values of the element positions in [m]. indspos returns longitudinal positions of accelerator lattice elements.
%  Return value is a vector of positions S at the entrance of each element
%  specified by its number in REFPTS. indspos returns longitudinal positions of accelerator lattice elements.
%  Return value is a vector of positions S at the entrance of each element
...
N_XSRbend = 14; 
% bend slice 2 in the XSR dipole bend, so we get the value at the start of this element where XSR is
...
grid on
plot(findspos(THERING,I_bends4x(14)),ROUT5(3,106),'ok')
plot(S_BPM2_09,ROUT5(3,I_bpms(N_BPM2_09)),'*r')
plot(S_BPM2_10,ROUT5(3,I_bpms(N_BPM2_10)),'*r')
...
hold off
%find the correctors
I_vcm = findcells(THERING,'FamName','COR');
S_vcm = findspos(THERING,I_vcm);
plot(spos(I_vcm),zeros(length(spos(I_vcm))),'ks') % plot the correctors